...
def test_final_exe_port_conflict():
    """最终测试打包后的exe文件端口冲突解决功能"""
    print("=== 最终测试打包后的exe文件端口冲突解决功能 ===")
    ...
    try:
        ...
        try:
            ...
            if "正在启动服务器" in stdout:
                print("   ✓ main.py已执行")
            else:
                print("   ✗ main.py未执行")
                
            if "检查端口 8000 是否被占用" in stdout:
                print("   ✓ 端口检测代码已执行")
            else:
                print("   ✗ 端口检测代码未执行")
            ...
            if "端口 8000 已释放" in stdout:
                print("   ✓ 端口已释放")
            else:
                print("   ✗ 端口未释放")
                
            if "启动服务器: 127.0.0.1:8000" in stdout:
                print("   ✓ 服务器已启动")
            else:
                print("   ✗ 服务器未启动")
